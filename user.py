...
Enter the option:
    ...
    6. Query all records from users table.
...
def open_csv_file(File: str) -> list[tuple]:
...
def insert_users(conn, db_data) -> None:
...
def select_all_records(conn) -> None: 
...
def update_user(conn, column_name: str, user_id: int, column_value: str | int) -> None:
...
def delete_user_id(conn, user_id: int) -> None:
    """ deletes single record from table    
    Args:
        conn (sqlite3 connection): sqlite3 connection object
        user_id (int): id of user
    Returns:
...
def delete_all_records(conn) -> None:
    ...
    conn.commit()
    print("Successfully deteletd all records from user table.")
...
def main():
    ...
    while True:
        ...
        if user_input == "1":
            ...
            confirmation = input("Are you sure?? press y or Yes to continue. or, press n or No to skip.")
